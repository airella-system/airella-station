...
bool ApiClass::registerSensor(const char *type)
{
    ...
    String url = String(apiUrlBase) + "/stations/" + Config::instance().getApiStationId() + "/sensors";
...
bool ApiClass::publishMeasurement(String sensor, double value)
{
    ...
    {
        String apiUrlBase = Config::instance().getApiUrl();
        String url = apiUrlBase + "/stations/" + Config::instance().getApiStationId() + "/sensors/" + sensor + "/measurements";
...
void ApiClass::configUpdated()
{
    ...
    if (!this->isRegistered() && !Config::instance().getRegistratonToken().equals(""))
    {
