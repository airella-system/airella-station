...
void WeatherSensor::init() {
    ...
    if(!i2cBus.begin(BME_SDA, BME_SCL, 100000)) {
        Logger::error("[WeatherSensor] Unable to initialize sensor bus communication.");
        return;
    }
...
bool WeatherSensor::isInit() {
    return initialized;
}

float WeatherSensor::getTemperature() {
    if(!initialized) {
        Logger::error("[WeatherSensor] Unable to get temperature value - sensor is uninitialized.");
    }
...
float WeatherSensor::getPreassure() {
    if(!initialized) {
        Logger::error("[WeatherSensor] Unable to get preassure value - sensor is uninitialized.");
    }
...
float WeatherSensor::getHumidity() {
    if(!initialized) {
        Logger::error("[WeatherSensor] Unable to get humidity value - sensor is uninitialized.");
    }
